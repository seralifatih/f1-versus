...
// Sidecar marker holding the F1DB release tag that produced the cached
// f1db.db. Lets the next sync run (and the build-info writer) recover the
// version without re-hitting the GitHub API.
const SOURCE_TAG_FILE = join(CACHE_DIR, 'f1db.tag')
const BUILD_INFO_FILE = join(process.cwd(), 'lib', 'build-info.ts')
...
async function main() {
  ...
  const tag = await downloadLatestRelease()
  await unzipDb()
...
async function downloadLatestRelease(): Promise<string> {
  if (existsSync(SOURCE_DB)) {
    ...
    if (age < 24) {
      const cachedTag = existsSync(SOURCE_TAG_FILE)
        ? readFileSync(SOURCE_TAG_FILE, 'utf8').trim()
        : 'unknown'
      console.log(`✓ Using cached f1db.db (${age.toFixed(1)}h old, ${cachedTag}). Delete .cache/f1db to refresh.`)
      return cachedTag
    }
  }
  ...
  writeFileSync(SOURCE_TAG_FILE, release.tag_name)
...
// ────────────────────────────────────────────────────────────────────────────
// Build-info regeneration
// ────────────────────────────────────────────────────────────────────────────

// Rewrites lib/build-info.ts so the LiveStats hero + Footer surface the
// numbers from this sync run. SEASON_STATUS stays manual — it encodes the
// "RD x/y" line which doesn't have a clean derivation from F1DB alone
// (round numbers shift around postponements). Everything else is derived
// directly from f1db.db + driver_stats.db.
function writeBuildInfo(src: Database.Database, out: Database.Database, tag: string) {
  // Drivers in the ranking = rows we just wrote with eraId='all', i.e. after
  // MIN_STARTS + Indy filter. Matches what the user actually sees on /.
  const driverCount = (
    out.prepare(`SELECT COUNT(*) AS n FROM driver_stats WHERE eraId = 'all'`).get() as { n: number }
  ).n

  const seasonCount = (
    src.prepare(`SELECT COUNT(DISTINCT year) AS n FROM season`).get() as { n: number }
  ).n

  // Indy filter mirrors the ranking pipeline: counts only non-Indy F1 races,
  // so the "Races" total on the hero matches the universe the ranking uses.
  const raceCount = (
    src
      .prepare(`SELECT COUNT(*) AS n FROM race r WHERE ${NON_INDY}`)
      .get() as { n: number }
  ).n

  // Existing SEASON_STATUS + APP_VERSION are preserved across regenerations.
  // We parse them out of the current file rather than re-declaring defaults,
  // so manual bumps don't get clobbered.
  const prev = existsSync(BUILD_INFO_FILE) ? readFileSync(BUILD_INFO_FILE, 'utf8') : ''
  const seasonStatus = prev.match(/SEASON_STATUS = '([^']*)'/)?.[1] ?? 'F1 — RD ?/?'
  const appVersion = prev.match(/APP_VERSION = '([^']*)'/)?.[1] ?? 'v0.1.0'

  const today = new Date()
  const syncDate = `${today.getUTCFullYear()}.${String(today.getUTCMonth() + 1).padStart(2, '0')}.${String(today.getUTCDate()).padStart(2, '0')}`
  const versionLabel = tag === 'unknown' ? 'F1DB (cached)' : `F1DB ${tag}`

  const contents = `// AUTO-GENERATED by scripts/sync-f1db.ts. Do not edit by hand —
...
export const BUILD_DATA_VERSION = '${versionLabel}'
export const BUILD_DATA_SYNC = '${syncDate}'
export const APP_VERSION = '${appVersion}'
export const SEASON_STATUS = '${seasonStatus}'
...
export const TOTAL_DRIVERS = ${driverCount}
export const TOTAL_SEASONS = ${seasonCount}
export const TOTAL_RACES = ${raceCount}
`

  writeFileSync(BUILD_INFO_FILE, contents)
  console.log(`\n✓ Wrote ${BUILD_INFO_FILE}`)
  console.log(`  ${versionLabel} · ${syncDate} · ${driverCount} drivers · ${seasonCount} seasons · ${raceCount} races`)
}
